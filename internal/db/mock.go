...
func (db *DBMock) GetUserById(id int) (api.User, error) {
	idx := slices.IndexFunc(db.users, func (u api.User) bool { return u.Id == id })
	if idx == -1 {
...
func (db *DBMock) GetUserByName(name string) (api.User, error) {
...
func (db *DBMock) DeleteUserById(id int) error {
	idx := slices.IndexFunc(db.users, func (u api.User) bool { return u.Id == id })
	if idx == -1 {
...
func (db *DBMock) DeleteUserByName(name string) error {
	idx := slices.IndexFunc(db.users, func (u api.User) bool { return u.Name == name })
	if idx == -1 {
		return ErrUserNotFound
	}
